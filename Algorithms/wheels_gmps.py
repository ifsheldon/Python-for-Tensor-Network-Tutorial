...
def acc_from_saved_gmps(samples, labels, para, paraMPS, batch_size=5000, pos=None):
    ...
    return acc_saved_gmpsc_by_file_names(
        samples, labels, gmps_names, batch_size=batch_size, pos=pos
    )


def acc_saved_gmpsc_by_file_names(
    samples, labels, gmps_files, batch_size=5000, pos=None, classes=None
):
    # pos: 用于实现特征选择后的nll计算
    if classes is None:
    ...
    for digit in range(len(classes)):
        ...
        while samples_tmp.numel() > 0:
            ...
            samples_ = feature_map(
                samples_, paraMPS["feature_map"], {"d": paraMPS["d"], "theta": paraMPS["theta"]}
            )
            if pos is None:
...
def OEE_variation_one_qubit_measurement(mps, features_vecs, pos=None, OEE_eps=-1):
    ...
    OEE_sum = OEE.sum()
    dOEE = tc.zeros(
        (features_vecs.shape[1],), device=features_vecs.device, dtype=features_vecs.dtype
    )
...
def OEE_variation_one_qubit_measurement_simple(mps, features_vecs):
    # feature_vecs.shape = (d, num_f)
    pos = tc.arange(features_vecs.shape[1])
    OEE = mps.entanglement_ent_onsite()
    OEE_sum = OEE.sum()
...
def generate_from_onebody_rho(rho, para_mps, para_g):
    ...
    sz = tc.tensor([[0, 0], [0, 1]], dtype=dtype, device=device)
    if para_g["way"] == "mz":
        if single_rho:
        ...
        vec = feature_map(
            state.reshape((-1,)),
            para_mps["feature_map"],
            para={"d": para_mps["d"], "theta": para_mps["theta"]},
        )
        vec = vec.squeeze()
        ...
        if single_rho:
            state = tc.arccos(tc.abs(v[0, -1])) / (para_mps["theta"] * np.pi / 2)
        else:
            state = tc.arccos(tc.abs(v[:, 0, -1])) / (para_mps["theta"] * np.pi / 2)
        vec = feature_map(
            state.reshape((-1,)),
        ...
        if single_rho:
            lm = rho.diag()
            state = choices([0, 1], (lm).cpu().numpy(), k=1)[0]
            vec = state
        else:
            state = list()
            for n in range(rho.shape[0]):
                lm = rho[n].diag()
                state.append(choices([0, 1], (lm).cpu().numpy(), k=1)[0])
