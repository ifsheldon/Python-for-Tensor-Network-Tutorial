...
from Library.MatrixProductState import MPS_tebd, generative_MPS, ResMPS_basic, activated_ResMPS
...
def ResMPS_classifier(train_loader, test_loader, para=None, paraMPS=None):
    if para is None:
        para = dict()
    if paraMPS is None:
        paraMPS = dict()
    ...
    for t in range(para["it_time"]):
        ...
        for nb, (img, lb) in enumerate(train_loader):
            vecs = df.feature_map(
                img.to(device=mps.device, dtype=mps.dtype),
        ...
        with tc.no_grad():
            for nt, (imgt, lbt) in enumerate(test_loader):
                vecs = df.feature_map(
                    imgt.to(device=mps.device, dtype=mps.dtype),
                    which=para["feature_map"],
                    para={"d": paraMPS["d"]},
                )
    ...
    info = {"train_acc": tc.tensor(train_acc), "test_acc": tc.tensor(test_acc)}
    return info, mps
...
def GMPS_train(samples, tensors=None, para=None, paraMPS=None):
    ...
    samples = df.feature_map(
        samples, paraMPS["feature_map"], {"d": paraMPS["d"], "theta": paraMPS["theta"]}
    )
    para["num_samples"], paraMPS["d"], paraMPS["length"] = samples.shape
    ...
    mps.center_orthogonalization(0, way="qr", dc=-1, normalize=True)
    ...
    for t in range(para["sweepTime"]):
        for n in range(paraMPS["length"]):
            ...
            if n < paraMPS["length"] - 1:
                mps.move_center_one_step("right", "qr", -1, normalize=True)
        ...
        for n in range(paraMPS["length"] - 1, -1, -1):
            mps.grad_update_MPS_tensor_by_env(lr)
            if n > 0:
                mps.move_center_one_step("left", "qr", -1, normalize=True)
                mps.update_vecsR_n(n)
            else:
                mps.normalize_central_tensor()
        if ((t + 1) % para["save_dt"] == 0) or (t == (para["sweepTime"] - 1)):
            save(
                para["save_dir"],
                para["save_name"],
...
def GMPS_classification(trainset, testset, para=None, paraMPS=None):
    ...
    for digit in range(len(classes)):
        ...
        if samples_now.numel() == 0:
            ...
            print("Training GMPS for digits " + str(classes[digit]))
            para["save_name"] = wf.gmps_save_name1(classes[digit], paraMPS)
            ...
            if type(data) is tuple:
                ...
                para["sweepTime"] = max(para["sweepTime"], para["sweepTimeNoData"])
            if tensors is not None:
                print("Load existing sample...")
            else:
                print("Train new GMPS ...")
            GMPS_train(
                samples_now.to(device=para["device"], dtype=para["dtype"]), tensors, para, paraMPS
            )
            tc.cuda.empty_cache()
    acc_train = wf.acc_from_saved_gmps(samples, labels, para, paraMPS)
    acc_test = wf.acc_from_saved_gmps(samples_t, labels_t, para, paraMPS)
    print("Train accuracy = %g, test accuracy = %g" % (acc_train, acc_test))
    return acc_train, acc_test


def GMPSC_feature_select(trainset, testset, para=None, paraMPS=None):
    """
    :param trainset: 训练集
    :param testset: 测试集
    :param para: 算法参数
    ...
    for digit in range(len(classes)):
        ...
        if samples_now.numel() == 0:
            ...
            if tensors is not None:
                ...
                print("Train new GMPS ...")
            info1, info2 = GMPS_train_feature_selection(
                samples_now.to(device=para["device"], dtype=para["dtype"]), tensors, para, paraMPS
            )[2:]
    ...
    if para["calcu_acc_no_select"]:
        ...
        fprint(
            "With all feature selected, train accuracy = %g, test accuracy = %g"
            % (acc_train, acc_test),
            file="acc.log",
        )
    else:
    ...
    fprint(
        "With %i feature selected, train accuracy = %g, test accuracy = %g"
        % (para["num_feature"], acc_train_fs, acc_test_fs),
        file="acc.log",
    )
...
def tebd(hamilts, pos, mps_tensors=None, para=None, paraMPS=None, output=None):
    """
    ...
    if type(hamilts) is tc.Tensor:
        hamilts = hamilts.to(device=para["device"], dtype=para["dtype"])
        paraMPS["d"] = hamilts.shape[0]
    else:
        hamilts = [ham.to(device=para["device"], dtype=para["dtype"]) for ham in hamilts]
        paraMPS["d"] = hamilts[0].shape[0]

    mps = MPS_tebd(tensors=mps_tensors, para=paraMPS)
    mps.center_orthogonalization(pos[0][-1], way="svd", dc=paraMPS["chi"])
    mps.normalize_central_tensor()
    ...
    beta = 0.0
    t_tau = 0
    eb = tc.zeros((len(pos),), device=para["device"], dtype=para["dtype"])
    gr = (
        tc.eye(paraMPS["d"] ** 2, device=para["device"], dtype=para["dtype"])
        .reshape([paraMPS["d"]] * 4)
        .permute(0, 2, 3, 1)
        .reshape(paraMPS["d"], paraMPS["d"] ** 2, paraMPS["d"])
    )
    for t in range(para["time_it"]):
        for p in range(len(pos)):
            if abs(mps.center - pos[p][0]) < abs(mps.center - pos[p][1]):
            ...
            if p == (len(pos) - 1):
                ...
                pos_next = pos[p + 1]
            direction = wh.find_optimal_new_center(pos[p][0], pos[p][1], pos_next[0], pos_next[1])
            if direction == "lr":
                mps.orthogonalize_n1_n2(pos[p][1], pos[p][0], way="qr", dc=-1, normalize=False)
                mps.orthogonalize_n1_n2(
                    pos[p][0], pos[p][1], way="svd", dc=paraMPS["chi"], normalize=False
                )
                mps.center = pos[p][1]
            else:
                mps.orthogonalize_n1_n2(pos[p][0], pos[p][1], way="qr", dc=-1, normalize=False)
                mps.orthogonalize_n1_n2(
                    pos[p][1], pos[p][0], way="svd", dc=paraMPS["chi"], normalize=False
                )
                mps.center = pos[p][0]
            mps.normalize_central_tensor()
        beta += tau
        ...
        if ((t_tau > para["t_tau_min"]) and ((t + 1) % para["time_ob"] == 0)) or (
            t == (para["time_it"] - 1)
        ):
            eb1 = mps.calculate_local_energies(hamilts, pos)
            fprint(
                "For beta = %g, average E = %g" % (beta, eb1.sum() / len(mps.tensors)),
                file=para["log_file"],
                print_screen=para["print"],
            )
            deb = (eb1 - eb).abs().sum() / eb.numel()
            eb = eb1
            if (deb < para["e0_eps"]) or (t == (para["time_it"] - 1)):
                mps.save(file=para["save_file"])
                tau *= 0.5
                t_tau = 0
                if (tau < para["tau_min"]) or (t == (para["time_it"] - 1)):
                    fprint(
                ...
                gates = wh.hamilts2gates(hamilts, tau)
                fprint(
                    "Reduce tau to %g..." % tau, file=para["log_file"], print_screen=para["print"]
                )
    if output is None:
        return None
    out = dict()
...
def tebd_spin_chain(mps_tensors=None, para=None, paraMPS=None, output=None):
    para0 = {
        "jxy": 1,
        "jz": 1,
        "hx": 0,
