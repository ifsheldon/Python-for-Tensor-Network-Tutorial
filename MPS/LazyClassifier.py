...
train_dataset, test_dataset = df.load_mnist(dataset, process={"classes": classes})
samples, labels = df.dataset2tensors(train_dataset)
samples, labels = df.select_num_samples(samples, labels, num, classes)
...
acc = knn_classifier(1, samples.numpy(), samples.numpy(), labels.numpy(), labels.numpy())[0]
acc_t = knn_classifier(1, samples_t.numpy(), samples.numpy(), labels.numpy(), labels_t.numpy())[0]
print("KNN (K=1) classifier:")
print("Train and Test accuracy = %g, %g" % (acc, acc_t))

acc = knn_classifier(200, samples.numpy(), samples.numpy(), labels.numpy(), labels.numpy())[0]
acc_t = knn_classifier(200, samples_t.numpy(), samples.numpy(), labels.numpy(), labels_t.numpy())[0]
print("KNN (K=200) classifier:")
print("Train and Test accuracy = %g, %g" % (acc, acc_t))

acc = lazy_classifier(samples, samples, labels, labels, {"kernel": "chebyshev"})[0]
acc_t = lazy_classifier(samples_t, samples, labels, labels_t, {"kernel": "chebyshev"})[0]
print("Lazy classifier with Chebyshev kernel:")
print("Train and Test accuracy = %g, %g" % (acc.item(), acc_t.item()))

acc = lazy_classifier(samples, samples, labels, labels, {"kernel": "cossin-chebyshev"})[0]
acc_t = lazy_classifier(samples_t, samples, labels, labels_t, {"kernel": "cossin-chebyshev"})[0]
print("Lazy classifier with Chebyshev NLF(cos-sin) kernel:")
print("Train and Test accuracy = %g, %g" % (acc.item(), acc_t.item()))

acc = lazy_classifier(samples, samples, labels, labels, {"kernel": "euclidean"})[0]
acc_t = lazy_classifier(samples_t, samples, labels, labels_t, {"kernel": "euclidean"})[0]
print("Lazy classifier with euclidean kernel:")
print("Train and Test accuracy = %g, %g" % (acc.item(), acc_t.item()))

acc = lazy_classifier(samples, samples, labels, labels, {"kernel": "cossin"})[0]
acc_t = lazy_classifier(samples_t, samples, labels, labels_t, {"kernel": "cossin"})[0]
...
print(
    "Train and test accuracy of GMPSC "
    "(d=%i, chi=%i) = %g and %g" % (paraMPS["d"], paraMPS["chi"], acc_mps[0], acc_mps[1])
)
