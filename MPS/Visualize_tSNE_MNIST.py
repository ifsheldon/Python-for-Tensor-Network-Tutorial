...
def visualize_tsne(samples, labels=None, para=None):
    ...
    if para["metric"] == "precomputed":
        para["init"] = "random"
    if labels is None:
        labels = tc.zeros(samples.shape[0], dtype=tc.int64)
    ...
    samples1 = tsne.fit_transform(samples.reshape(samples.shape[0], -1))
    x_min = samples1[:, 0].min(0)
    x_max = samples1[:, 0].max(0)
    samples1[:, 0] = (samples1[:, 0] - x_min) / (x_max - x_min)  # 归一化
    y_min = samples1[:, 1].min(0)
    y_max = samples1[:, 1].max(0)
    samples1[:, 1] = (samples1[:, 1] - y_min) / (y_max - y_min)  # 归一化
    plt.figure(figsize=(8, 8))
    color_list = [plt.cm.tab10(n) for n in range(labels.max() + 1)]
...
# 从训练集随机选出num个样本
train_dataset = load_mnist(dataset, process={"classes": classes})[0]
...
visualize_tsne(
    samples.numpy(),
    labels.numpy(),
    para={"metric": "euclidean", "perplexity": perplexity, "save_name": save_path + "EU.png"},
)
...
visualize_tsne(
    dis.numpy(),
    labels.numpy(),
    para={"metric": "precomputed", "perplexity": perplexity, "save_name": save_path + "NLF.png"},
)
...
for beta in [1.01, 1.05, 1.1, 1.3]:
    ...
    visualize_tsne(
        dis_resc.numpy(),
        labels.numpy(),
        para={"metric": "precomputed", "perplexity": perplexity, "save_name": fname},
    )
