...
dataset = "fmnist"
category = [0]
para = {"lr": 0.1, "sweepTime": 10, "feature_map": "cossin", "dtype": tc.float32}

paraMPS = {"length": 784, "d": 2, "chi": 64}

train_dataset, test_dataset = load_mnist(dataset, process={"classes": category})
...
mps, para, info = GMPS_train(train_img, para=para, paraMPS=paraMPS)
nll_test = mps.evaluate_nll(test_img, average=True)


plot(
    tc.arange(len(info["nll"])),
    info["nll"],
