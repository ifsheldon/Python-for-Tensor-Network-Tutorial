...
tensors = (
    [tc.randn(1, d, chi, dtype=dtype)]
    + [tc.randn(chi, d, chi, dtype=dtype) for _ in range(length - 2)]
...
psi1 = psi1.permute([0, length + 1] + list(range(1, length + 1))).reshape(chi, chi, -1)
