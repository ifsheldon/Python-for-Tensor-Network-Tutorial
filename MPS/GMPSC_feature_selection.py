...
from Algorithms.wheels_gmps import acc_saved_gmpsc_by_file_names, gmps_save_name1
...
for digit in range(10):
    para["save_name"] = (
        r"GMPS_chi12_theta1.0_FM_"
        r"cossin_digit_%i" % digit
    )
    tensors, paraMPS = bf.load(
        os.path.join(para["save_dir"], para["save_name"]), ["tensors", "paraMPS"]
    )
    gmps = mps.generative_MPS(tensors, paraMPS)
    gmps.correct_device()
    ent = gmps.entanglement_ent_onsite()
    OEE_list.append(ent)
    gmps_names.append(os.path.join(para["save_dir"], para["save_name"]))
bf.show_multiple_images(OEE_list, (2, 5), list(range(10)), img_size=(28, 28))

print("Calculating accuracies of GMPSC with all features...")
acc_train = acc_saved_gmpsc_by_file_names(
    train_img.to(device=bf.choose_device(), dtype=paraMPS["dtype"]),
    labels.to(device=bf.choose_device()),
    gmps_files=gmps_names,
    classes=list(range(10)),
)
acc_test = acc_saved_gmpsc_by_file_names(
    test_img.to(device=bf.choose_device(), dtype=paraMPS["dtype"]),
    labels_t.to(device=bf.choose_device()),
    gmps_files=gmps_names,
    classes=list(range(10)),
)
bf.fprint("Train & test acc = %g, %g" % (acc_train.item(), acc_test.item()), file="results.log")

OEE_sum = sum(OEE_list)
selected_pos = tc.argsort(OEE_sum, descending=True)
for num in [15, 20, 50, 100, 200, 300, 400, 500, 600]:
    bf.fprint(
        "Selecting %i features and classify with the RMD of full GMPS ..." % num, file="results.log"
    )
    acc_train = acc_saved_gmpsc_by_file_names(
        train_img, labels, gmps_files=gmps_names, pos=selected_pos[:num], classes=list(range(10))
    )
    acc_test = acc_saved_gmpsc_by_file_names(
        test_img, labels_t, gmps_files=gmps_names, pos=selected_pos[:num], classes=list(range(10))
    )
    bf.fprint("train & test acc = %g, %g" % (acc_train.item(), acc_test.item()), file="results.log")
...
for num in [15, 20, 50, 100, 200, 300, 400, 500, 600]:
    ...
    for digit in range(10):
        print("Class: %i" % digit)
        samples = train_img[labels == digit][:, selected_pos[:num]]
        para_tmp["save_name"] = gmps_save_name1(digit, paraMPS_tmp)
        para_tmp["save_name"] += "_selected%i" % num
        gmps_names.append(os.path.join(para_tmp["save_dir"], para_tmp["save_name"]))
        GMPS_train(samples, para=para_tmp, paraMPS=paraMPS_tmp)
        train_tmp.append(samples)
        test_tmp.append(
            (test_img[labels_t == digit][:, selected_pos[:num]]).to(
                device=paraMPS["device"], dtype=paraMPS["dtype"]
            )
        )
        train_lbs.append(
            tc.ones(len(train_tmp[-1]), device=paraMPS["device"], dtype=tc.int64) * digit
        )
        test_lbs.append(
            tc.ones(len(test_tmp[-1]), device=paraMPS["device"], dtype=tc.int64) * digit
        )
    acc_train = acc_saved_gmpsc_by_file_names(
        tc.cat(train_tmp, dim=0),
        tc.cat(train_lbs, dim=0),
        gmps_files=gmps_names,
        classes=list(range(10)),
    )
    acc_test = acc_saved_gmpsc_by_file_names(
        tc.cat(test_tmp, dim=0),
        tc.cat(test_lbs, dim=0),
        gmps_files=gmps_names,
        classes=list(range(10)),
    )
    bf.fprint(
        "Train & test acc (%i features) = %g, %g" % (num, acc_train.item(), acc_test.item()),
        file="results.log",
    )
