...
sample, _ = load_mnist("mnist", test=False, process={"classes": [0], "return_tensor": True})[0]
sample = sample[sample_index].reshape(1, -1)

tensors, paraMPS = bf.load(os.path.join(gmps_path, gmps_file), ["tensors", "paraMPS"])
...
t0 = time.time()
dOEE = wg.OEE_variation_one_qubit_measurement_simple(gmps, img_vecs.squeeze())
print("Time cost: %i" % (time.time() - t0))
t0 = time.time()
dOEE1 = wg.OEE_variation_one_qubit_measurement(gmps, img_vecs.squeeze(), OEE_eps=1e-2)
print("Time cost: %i" % (time.time() - t0))
bf.show_multiple_images(
    [sample.reshape(28, 28), dOEE.reshape(28, 28), dOEE1.reshape(28, 28)], lxy=[1, 3]
)
