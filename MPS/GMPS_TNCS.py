...
from Algorithms.MPS_algo import generate_sample_by_gmps, input_GMPS
...
file = "GMPS_chi64_theta1.0_FM_cossin_class_" + str(category)
num_sp = 1  # 生成每个像素时的采样次数
# 已知特征数量
nf = [1, 2, 5, 10, 15, 20, 25, 30, 40, 50, 100]

print("Load fashion-MNIST dataset and image (#%i)..." % which_img)
imgList0, imgList1 = list(), list()
imgList2, imgList3 = list(), list()

img0 = df.load_samples_mnist("fmnist", num=which_img, pos="pos", process={"classes": [category]})
img0 = img0.reshape(
    -1,
)
...
imgRC0 = generate_sample_by_gmps(gmps.clone_gmps(), paraG={"way": "inverse", "num_s": num_sp})
...
for num_f in nf:
    ...
    img1 = img0.clone()
    oee_order1 = bf.supplementary(tc.arange(784), oee_order[:num_f])
    oee_order1 = sorted(list(oee_order1))
    img1[oee_order1] = 0.0
    imgList0.append(img1.reshape(28, 28))
    img_RC = generate_sample_by_gmps(
        gmps.clone_gmps(), img1, oee_order1, paraG={"way": "inverse", "num_s": num_sp}
    )
    imgList1.append(img_RC.reshape(28, 28))

    print("Selecting by ESOP...")
    img1 = img0.clone()
    eosp_order1 = bf.supplementary(tc.arange(784), gmps.eosp_ordering[:num_f])
    eosp_order1 = list(eosp_order1)
    img1[eosp_order1] = 0.0
    imgList2.append(img1.reshape(28, 28))
    img_RC = generate_sample_by_gmps(
        gmps.clone_gmps(), img1, eosp_order1, paraG={"way": "inverse", "num_s": num_sp}
    )
    imgList3.append(img_RC.reshape(28, 28))

bf.show_multiple_images(imgList0 + imgList1 + imgList2 + imgList3, lxy=(4, -1))
