...
series = series_sin_cos(x, tc.randn(para["order_g"]), tc.randn(para["order_g"]))

num_train = int(series.numel() * 0.8)
(f_train,) = plt.plot(list(range(num_train)), series[:num_train])
(f_test,) = plt.plot(
    list(range(num_train - 1, series.numel(), 1)), series[num_train - 1 :], linewidth="3", color="r"
)
...
print("预处理数据")
series1 = series.to(dtype=para_adqc["dtype"], device=para_adqc["device"])
...
(l10,) = plt.plot(epochs, np.array(results_lstm["train_loss"]), linestyle="--", color="r")
(l11,) = plt.plot(epochs, np.array(results_lstm["test_loss"]), color="r")
(l12,) = plt.plot(epochs, np.array(results_adqc["train_loss"]), linestyle="--", color="b")
(l13,) = plt.plot(epochs, np.array(results_adqc["test_loss"]), color="b")
(l14,) = plt.plot(epochs, np.array(results_qrnn["train_loss"]), linestyle="--", color="g")
