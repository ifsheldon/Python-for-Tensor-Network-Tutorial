...
train_samples, train_labels, test_samples, test_labels = df.split_dataset_train_test(
    samples, targets
)
trainset = TensorDataset(train_samples, train_labels)
testset = TensorDataset(test_samples, test_labels)
trainset = DataLoader(trainset, para["n_img"], shuffle=True)
testset = DataLoader(testset, para["n_img"], shuffle=False)

print("训练FC2分类器")
_, out1, _ = FC2_classifier(trainset, testset, num_classes=3, length=length, para=para_nn)
print("训练ADQC分类器")
_, out2, _ = ADQC_classifier(trainset, testset, num_classes=3, length=length, para=para_qc)

print(
    "训练完成\n 训练集精度：FC2 = %g,\t ADQC = %g" % (out1["train_acc"][-1], out2["train_acc"][-1])
)
print(" 测试集精度：FC2 = %g,\t ADQC = %g" % (out1["test_acc"][-1], out2["test_acc"][-1]))
