...
def plot_points_and_line(
    data, colours, slop, intercept, title="", save_path=None, file_name="fig", show=False
):
    plt.scatter(data[:, 0], data[:, 1], c=colours)
    xm = data.abs().max()
    plt.xlim(-xm, xm)  # 设置横坐标范围
    plt.ylim(-xm, xm)  # 设置纵坐标范围
    plt.axline((0, intercept), slope=slop, color="gray", linestyle="--", linewidth=1)
    plt.title(title)
    if save_path:
...
def plot_points_and_circle(
    data, colours, radius, title="", save_path=None, file_name="fig", show=False
):
    fig, ax = plt.subplots()
    ...
    plt.xlim(-xm, xm)  # 设置横坐标范围
    plt.ylim(-xm, xm)  # 设置纵坐标范围
    circle = plt.Circle((0, 0), radius, fill=False, color="gray", linestyle="--", linewidth=1)
