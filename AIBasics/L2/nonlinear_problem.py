...
colors = ["red" if labels[i] > 0 else "blue" for i in range(labels.numel())]
plot_points_and_circle(points, colors, r0, "Ground-truth classification", show=True)

print("利用nn.Linear建立分类器")
net = nn.Linear(2, 1)

print("绘制初始（随机）Linear分类器对应的分类边界")
slp = -(net.state_dict()["weight"][0, 0] / net.state_dict()["weight"][0, 1]).item()
intcpt = -(net.state_dict()["bias"] / net.state_dict()["weight"][0, 1]).item()
plot_points_and_line(points, colors, slp, intcpt, "Initial classification")
...
for t in range(epoch_train):  # 循环优化（训练）
    ...
    if (t + 1) % epoch_check == 0:
        ...
        plot_points_and_line(
            points, colors, slp, intcpt, title, path, file_name=str(time.time()).replace(".", "")
        )
