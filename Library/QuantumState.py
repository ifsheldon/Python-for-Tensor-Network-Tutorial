...
class TensorPureState:
    def __init__(self, tensor=None, nq=None, device=None, dtype=None):
        self.rand_seed = None
        ...
        if nq is None:
            nq = 3
        if tensor is None:
            self.tensor = tc.randn([2] * nq, device=self.device, dtype=self.dtype)
            self.normalize(p=2)
        else:
            self.tensor = tensor
    ...
    def act_single_gate(self, gate, pos, pos_control=None):
        if bf.compare_iterables(pos, pos_control):
            sys.exit("warning in act_single_gate: repeated position(s) in pos and pos_control...")
        ...
        state1 = self.tensor.permute(perm).reshape(2**m_p, -1, 2**m_c)
        state1_ = state1[:, :, :-1]
        ...
        self.tensor = state1.permute(bf.inverse_permutation(perm))
    ...
    def bipartite_ent_entropy_all(self, eps=1e-14):
        ent = tc.zeros(self.tensor.ndimension() - 1, device=self.device, dtype=self.dtype)
    ...
    def sampling(self, n_shots=1024, position=None, basis=None, if_print=False, rand_seed=None):
