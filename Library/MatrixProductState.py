...
class MPS_basic:
    ...
    def bipartite_entanglement(self, nt, normalize=False):
        ...
        if self.center <= nt:
            ...
            lm = tc.linalg.svdvals(self.tensors[nt].reshape(-1, self.tensors[nt].shape[-1]))
        else:
            self.center_orthogonalization(nt + 1, "qr", dc=-1, normalize=normalize)
            lm = tc.linalg.svdvals(self.tensors[nt + 1].reshape(self.tensors[nt + 1].shape[0], -1))
        return lm
    ...
    def EOSP(self, num_f=-1, recalculate=False, clone=True, eps=1e-14):
        if clone:
    ...
    def EOSP_average_OEEs(self, num_f=-1, recalculate=False, clone=True, eps=1e-14):
    ...
    def EOSP_fast_eps(self, num_f=-1, oee_eps=1e-6, eps=1e-14):
        ...
        for n in range(num_f):
            ...
            if len(pos_rec) == 1:
                ...
                self.center_orthogonalization(c=0, way="qr", normalize=True)
                ...
                qs_num += OEE.sum().item() / OEE.numel() / np.log(self.tensors[0].shape[1])
                p_max = OEE.argmax()
                pos_eosp.append(pos_rec.pop(p_max))
                rho = self.one_body_RDM(p_max)
                lm, u = tc.linalg.eigh(rho)
                u = u[:, lm.argmax()]
                self.project_multi_qubits([p_max], u.reshape(-1, 1))
                pos_large_oee = [x for x in range(OEE.numel()) if OEE[x] > oee_eps]
                pos_large_oee.pop(p_max)
        pos_eosp = tc.tensor(pos_eosp, dtype=tc.int64, device=self.device)
        return pos_eosp, qs_num
        ...
        if (
            ...
            for n in range(num_f):
                ...
                if len(pos_rec) <= 1:
                    ...
                    self.center_orthogonalization(c=0, way="qr", normalize=True)
                    OEE = self.entanglement_ent_onsite(print_info=False)
                    eosp_oee_av.append(OEE.sum().item() / OEE.numel())
                    qs_num += eosp_oee_av[-1] / np.log(self.tensors[0].shape[1])
                    p_max = OEE.argmax()
                    pos_eosp.append(pos_rec.pop(p_max))
                    rho = self.one_body_RDM(p_max)
                    lm, u = tc.linalg.eigh(rho)
                    ...
                    self.project_multi_qubits([p_max], u.reshape(-1, 1))
            pos_eosp = tc.tensor(pos_eosp, dtype=tc.int64, device=self.device)
            eosp_oee_av = tc.tensor(eosp_oee_av, dtype=self.dtype, device=self.device)
    ...
    def entanglement_ent_onsite(self, which=None, eps=None, recalculate=False, print_info=True):
        ...
        if type(which) not in [tc.Tensor, np.ndarray, list, tuple] and (which in [None, "all"]):
    ...
    def one_body_RDM(self, nt):
        ...
        if self.center < -0.5:
            ...
            for n in range(nt):
                vl = tc.einsum("apb,cpd,ac->bd", self.tensors[n].conj(), self.tensors[n], vl)
                vl = vl / vl.norm()
            vr = tc.ones((1, 1), device=self.device, dtype=self.dtype)
            for n in range(self.length - 1, nt, -1):
                vr = tc.einsum("apb,cpd,bd->ac", self.tensors[n].conj(), self.tensors[n], vl)
                vr = vr / vr.norm()
            rho = tc.einsum("apb,cqd,ac,bd->pq", self.tensors[nt].conj(), self.tensors[nt], vl, vr)
        else:
            if self.center < nt:
                v = tc.eye(self.tensors[self.center].shape[0], device=self.device, dtype=self.dtype)
                for n in range(self.center, nt):
                    v = tc.einsum("apb,cpd,ac->bd", self.tensors[n].conj(), self.tensors[n], v)
                    v = v / v.norm()
                rho = tc.einsum("apb,cqb,ac->pq", self.tensors[nt].conj(), self.tensors[nt], v)
            else:
                v = tc.eye(
                    self.tensors[self.center].shape[-1], device=self.device, dtype=self.dtype
                )
                for n in range(self.center, nt, -1):
                    v = tc.einsum("apb,cpd,bd->ac", self.tensors[n].conj(), self.tensors[n], v)
                    v = v / v.norm()
                rho = tc.einsum("apb,aqd,bd->pq", self.tensors[nt].conj(), self.tensors[nt], v)
    ...
    def orthogonalize_left2right(self, nt, way, dc=-1, normalize=False):
        ...
        if way.lower() == "svd":
            ...
            if if_trun:
                ...
                r = tc.diag(lm[:dc]).to(device=self.device).mm(v[:dc, :].to(device=self.device))
            else:
        ...
        if normalize:
            r /= tc.norm(r)
        self.tensors[nt + 1] = tc.tensordot(r, self.tensors[nt + 1], [[1], [0]])
        return lm
        ...
        if len(self.tensors) > 1:
            if nt == 0:  # contract to 1st tensor
                ...
                self.tensors[nt - 1] = tc.tensordot(
                    self.tensors[nt - 1], self.tensors[nt], [[-1], [0]]
                )
            self.tensors.pop(nt)
    ...
    def Q_sparsity(self, recalculate=False, clone=True, eps=1e-14):
        if recalculate or (self.qs_number is None):
            if clone:
                mps1 = self.clone_gmps()
                mps1.eps = eps
                self.eosp_ordering = mps1.EOSP_self(num_f=-1, recalculate=recalculate)
    ...
    def save(self, path=None, file=None):
        bf.save(
            path,
            file,
            [self.tensors, self.para, self.properties()],
...
class ResMPS_basic(nn.Module, MPS_basic):
    def __init__(self, tensors=None, para=None):
        ...
        if self.para["last_fc"]:
            self.fc = nn.Linear(
                self.para["chi"], self.para["classes"], bias=self.para["bias_fc"]
        ...
        if (tensors is None) or (self.tensors[self.pos_c].ndimension != 4):
            ...
            if self.fc is None:
                self.tensors[self.pos_c] = tc.randn(s[0], s[1], self.para["classes"], s[2]).to(
                    device=self.device, dtype=self.dtype
                )
            else:
                self.tensors[self.pos_c] = tc.randn(s[0], s[1], self.para["chi"], s[2]).to(
                    device=self.device, dtype=self.dtype
                )
        ...
        if self.para["bias"]:
            self.bias = nn.Parameter(
                self.para["eps"]
                * tc.randn((self.length, self.para["chi"]), device=self.device, dtype=self.dtype)
                * self.para["eps"],
                requires_grad=True,
            )
        else:
    ...
    def forward(self, x, vL=None, vR=None):
        ...
        if vL is None:
            vL = tc.ones(
                (x.shape[0], self.tensors[0].shape[0]), device=self.device, dtype=self.dtype
            )
        ...
        if vR is None:
            vR = tc.ones(
                (x.shape[0], self.tensors[-1].shape[-1]), device=self.device, dtype=self.dtype
            )
        ...
        dv = tc.einsum("abcd,na,nb,nd->nc", self.tensors[self.pos_c], vL, x[:, :, self.pos_c], vR)
...
class activated_ResMPS(ResMPS_basic):
    def __init__(self, tensors=None, para=None):
        super(activated_ResMPS, self).__init__(tensors=tensors, para=para)
...
class generative_MPS(MPS_basic):
    def __init__(self, tensors=None, para=None, properties=None):
        super(generative_MPS, self).__init__(tensors=tensors, para=para, properties=properties)
        self.samples_v = None
        self.vecsL = None
        self.vecsR = None
    ...
    def add_mps_vecs(self, vecs, factors=1.0):
        ...
        for n in range(1, len(self.tensors) - 1):
            ...
            x1 = tc.zeros(
                (s[0] + tensors1[n].shape[0], s[1], s[2] + tensors1[n].shape[2]),
    ...
    def evaluate_nll(self, samples=None, average=False, update_vecs=True):
        ...
        if update_vecs:
            ...
            self.norms = tc.ones(
                (samples.shape[0], len(self.tensors)), device=self.device, dtype=self.dtype
            )
            center = max(0, self.center)
            for n in range(center):
                vl = tc.einsum("na,nb,abc->nc", vl, self.samples_v[:, :, n], self.tensors[n])
                self.norms[:, n] = vl.norm(dim=1)
                vl = tc.einsum("na,n->na", vl, 1 / (self.norms[:, n] + self.eps))
            for n in range(len(self.tensors) - 1, center, -1):
                vr = tc.einsum("nc,nb,abc->na", vr, self.samples_v[:, :, n], self.tensors[n])
                self.norms[:, n] = vr.norm(dim=1)
                vr = tc.einsum("na,n->na", vr, 1 / (self.norms[:, n] + self.eps))
            self.norms[:, center] = tc.einsum(
                "apb,na,np,nb->n", self.tensors[center], vl, self.samples_v[:, :, center], vr
            )
    ...
    def evaluate_nll_selected_features(self, samples_v, pos, average=False):
        ...
        self.norms = tc.ones(
            (samples_v.shape[0], len(self.tensors)), device=self.device, dtype=self.dtype
        )
        center = max(0, self.center)
        for n in range(center):
            if n in pos:
            ...
            vl = tc.einsum("nab,n->nab", vl, 1 / (self.norms[:, n] + self.eps))
        for n in range(self.para["length"] - 1, center, -1):
            ...
            vr = tc.einsum("nab,n->nab", vr, 1 / (self.norms[:, n] + self.eps))
        if center in pos:
            mat = tc.einsum("asb,ns->nab", self.tensors[center], samples_v[:, :, center])
            self.norms[:, center] = tc.einsum("nab,nac,ncd,nbd->n", mat.conj(), vl, mat, vr).abs()
        else:
            self.norms[:, center] = tc.einsum(
                "asb,csd,nac,nbd->n", self.tensors[center].conj(), self.tensors[center], vl, vr
            ).abs()
        nll = self.evaluate_nll_from_norms(average=average)
        return nll
    ...
    def evaluate_nll_from_norms(self, average=True):
        if average:
            return -2 * tc.log(self.norms.abs() + self.eps).sum() / self.norms.shape[0]
        else:
            return -2 * tc.log(self.norms.abs() + self.eps).sum(dim=1)
    ...
    def grad_update_MPS_tensor_by_env(self, lr, way="tsgo"):
        ...
        if way.lower() == "tsgo":
            ...
            proj = tc.dot(
                grad,
                self.tensors[self.center].reshape(
                    -1,
                ),
            ) * self.tensors[self.center].reshape(
                -1,
    ...
    def initialize_vecs_norms(self):
        ...
        self.norms = tc.ones((num_f, len(self.tensors)), device=self.device, dtype=self.dtype)
        self.vecsL = tc.ones((num_f, chi, len(self.tensors)), device=self.device, dtype=self.dtype)
    ...
    def obtain_env_center(self, n=None):
        ...
        c_env = tc.einsum("apb,napb->n", self.tensors[n], env)
        # 保证被除的数不接近于0
        c_env += tc.sign(c_env) * self.eps
        env = tc.einsum("napb,n->apb", env, 1 / c_env) / c_env.numel()
        return env
    ...
    def update_norms_center(self, center=None):
        ...
        self.norms[:, center] = tc.einsum(
            "apb,na,np,nb->n",
            self.tensors[center],
            self.vecsL[:, : s[0], center],
            self.samples_v[:, :, center],
            self.vecsR[:, : s[2], center],
        )
    ...
    def update_vecsL_n(self, n):
        ...
        self.vecsL[:, : s[2], n + 1] = tc.einsum(
            "na,nb,abc->nc", self.vecsL[:, : s[0], n], self.samples_v[:, :, n], self.tensors[n]
        )
        self.norms[:, n] = self.vecsL[:, : s[2], n + 1].norm(dim=1)
        self.vecsL[:, : s[2], n + 1] = tc.einsum(
            "na,n->na", self.vecsL[:, : s[2], n + 1], 1 / (self.norms[:, n] + self.eps)
        )

    def update_vecsR_n(self, n):
        s = self.tensors[n].shape
        self.vecsR[:, : s[0], n - 1] = tc.einsum(
            "nc,nb,abc->na", self.vecsR[:, : s[2], n], self.samples_v[:, :, n], self.tensors[n]
        )
        self.norms[:, n] = self.vecsR[:, : s[0], n - 1].norm(dim=1)
        self.vecsR[:, : s[0], n - 1] = tc.einsum(
            "na,n->na", self.vecsR[:, : s[0], n - 1], 1 / (self.norms[:, n] + self.eps)
        )
...
class MPS_tebd(MPS_basic):
    def __init__(self, tensors=None, para=None, properties=None):
        super(MPS_tebd, self).__init__(tensors=tensors, para=para, properties=properties)
...
def check_center_orthogonality(tensors, center, prt=False):
    ...
    for n in range(center):
        ...
        err[n] = (
            (tmp - tc.eye(tmp.shape[0], device=tensors[n].device, dtype=tensors[n].dtype))
            .norm(p=1)
            .item()
        )
...
def evaluate_nll(mps, samples=None, average=False):
    ...
    if samples is not None:
        if samples.ndimension() == 2:
            samples_v = df.feature_map(
                samples, mps.para["feature_map"], {"d": mps.para["d"], "theta": mps.para["theta"]}
            )
        else:
            assert samples.ndimension() == 3
            samples_v = samples
...
def inner_product(tensors0, tensors1, form="log"):
    ...
    v = tc.kron(v0, v1).reshape(
        [tensors0[0].shape[0], tensors1[0].shape[0], tensors0[0].shape[0], tensors1[0].shape[0]]
    )
    norm_list = list()
...
def norm_square(tensors, normalize=False, form="log"):
    ...
    v = tc.eye(tensors[0].shape[0] ** 2, dtype=tensors[0].dtype, device=tensors[0].device).reshape(
        [tensors[0].shape[0]] * 4
    )
...
def random_mps(length, d, chi, boundary="open", device=None, dtype=tc.float64):
    ...
    if boundary == "open":
        tensors = [tc.randn((chi, d, chi), device=device, dtype=dtype) for _ in range(length - 2)]
        return (
            [tc.randn((1, d, chi), device=device, dtype=dtype)]
            + tensors
            + [tc.randn((chi, d, 1), device=device, dtype=dtype)]
        )
    else:  # 周期边界MPS
        return [tc.randn((chi, d, chi), device=device, dtype=dtype) for _ in range(length)]
