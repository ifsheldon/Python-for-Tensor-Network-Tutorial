...
for chi_now in chi:
    ...
    paraMPS = {"chi": chi_now, "length": para["length"]}
    out = tebd_spin_chain(para=para, paraMPS=paraMPS, output=["eb"])
    E0_tebd.append(out["eb"].sum().item() / paraMPS["length"])
    print(
        "E0 per site = %g (relative error = %g)" % (E0_tebd[-1], abs((E0_tebd[-1] - E0_ed) / E0_ed))
    )
